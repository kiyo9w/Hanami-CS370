...
% --- MODIFICATIONS FOR OVERFLOW & TABLE PLACEMENT ---
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\usepackage{pdfpages}
\usepackage{ltxtable}
\usepackage{ltablex} % Should be loaded after array, if array is explicitly loaded. It is.
\usepackage{caption}
...
% --- ADDED PACKAGES ---
\usepackage{microtype} % Improves typography and helps prevent overfull hboxes
\usepackage{makecell}  % For multi-line cells in tables, e.g., for table headers
% \usepackage{afterpage} % For improved control over page breaks (Table III) - REMOVED for new strategy
\usepackage{stfloats}  % For better placement of double-column floats (Tables IV, V)
%\usepackage{dblfloatfix} % To fix issues with double-column float placement (Tables IV, V)
...
\lstset{
    ...
    % --- MODIFIED/ADDED LISTINGS OPTIONS ---
    breakatwhitespace=true, % Prefer breaking at whitespace
    breakindent=0pt,          % No indent for broken lines (or set to a small value like 10pt if preferred)
    keywordstyle=\color{blue},
    commentstyle=\color{green!40!black},
    stringstyle=\color{purple},
    identifierstyle=\color{black},
    emph={garden,species,seed,grow,bloom,water,root,style,branch,blossom,open,hidden,guarded}, % Custom keywords for Hanami
...
\begin{table}[htbp] % Placement suggestion: here, top, bottom, page
    ...
    \begin{tabular}{@{}ll@{}} % Use @{} to remove padding at edges. Reduced one l column as it was empty.
        \toprule
...
\begin{table}[H]
	...
	\footnotesize % Even smaller than \small to ensure it fits
...
\begin{figure}[H] % Consider [htbp] if [H] causes issues
        \centering
        \includegraphics[width=1\linewidth]{lexer_imp.png} % Ensure image path is correct
        \caption{Lexer Implementation}
        \label{fig:lexer_imp} % Changed label to be unique
    \end{figure}
...
\begin{figure}[H] % Consider [htbp]
    \centering
    \includegraphics[width=1\linewidth]{lexer_imp} % Note: Filename is 'lexer_imp', is this correct or should it be parser_imp.png? Assuming it's a placeholder.
    \caption{Parser Implementation}
    \label{fig:parser_imp} % Changed label
\end{figure}
...
\begin{figure}[H] % Consider [htbp]
        \centering
        \includegraphics[width=1\linewidth]{seman_imp.png} % Ensure image path is correct
        \caption{Semantic Analyzer Implementation}
        \label{fig:seman_imp} % Changed label
    \end{figure}
...
\begin{figure}[H] % Consider [htbp]
    \centering
    \includegraphics[width=1\linewidth]{codegen_flow.png} % Ensure image path is correct
    \caption{Code Generation Workflow}
    \label{fig:codegen_flow} % Changed label
\end{figure}
...
\begin{figure}[H] % Consider [htbp]
    \centering
    \includegraphics[width=1\linewidth]{buildsys.png} % Ensure image path is correct
    \caption{Build System Overview}
    \label{fig:buildsys} % Changed label
\end{figure}
...
% --- MODIFIED: Changed verbatim to lstlisting for Program 1 ---
...
% --- MODIFIED: Changed verbatim to lstlisting for Program 2 ---
...
% Create a section specifically for performance results with clear separation
...
% First table - made more compact to fit in two-column format
\begin{table}[H]
	\centering
	...
	\setlength{\tabcolsep}{3pt} % Very compact columns
...
% Second table - made more compact to fit in two-column format
\begin{table}[H]
	\centering
...
% Make sure to have these packages in your preamble:
...
% --- ADDED \FloatBarrier before subsequent paragraphs to ensure tables are placed ---
\FloatBarrier 
...
% --- MODIFIED: Changed verbatim to lstlisting ---
